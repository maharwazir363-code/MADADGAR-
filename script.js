/* ============================================================
   MADADGAR APP — JavaScript Core Engine (PART 1)
   ============================================================ */
...
/* ============================================================
   MADADGAR APP — JavaScript Core Engine (PART 2)
   ============================================================ */
...
function postCardHTML(post, opts = {}) {
  ...
  if (post.done) {
    inner += `<div class="done-badge" style="background:#25D366; color:white; padding:5px; text-align:center; font-weight:bold; margin-top:5px; border-radius:4px;">✓ ALHAMDULILLAH! KAAM HO GAYA</div>`;
  }

  if (post.category) {
    inner += `<div class="post-row bold" style="margin-top:8px;"><span class="ic">🛠️</span> <span>${escapeHtml(post.category)}</span></div>`;
  }
  ...
  inner += `<div class="post-meta" style="font-size:12px; color:#666; margin-top:5px;">${timeAgo(post.createdAt)} • ${post.viewCount || 0} views</div>`;
    ...
    if (!post.done) {
        ...
        inner += `
            <div class="post-actions" style="display: flex; flex-direction: column; gap: 5px; margin-top:10px;">
                <button type="button" class="action-btn" onclick="openChatWithUser('${post.username || 'test_user'}', '${escapeHtml(post.ownerName || 'User')}')" style="background-color: #007bff; color: white; padding:8px; border:none; border-radius:5px; cursor:pointer;">💬 Chat</button>
...
function renderReportedPosts() {
  ...
  container.innerHTML = reported.map((p) => {
      ...
      <div class="reported-post-item" style="padding:10px; border-bottom:1px solid #ddd; display:flex; justify-content:space-between; align-items:center;">
        ...
        <div>
          <span style="color:orange; margin-right:8px;">⚠️ ${p.reportsCount}</span>
          <button class="user-action-btn danger" data-act="delete-reported" data-id="${escapeHtml(p.id)}" style="background:#dc3545; color:white; border:none; padding:4px 8px; border-radius:4px;">🗑 Delete</button>
        </div>
      </div>`;
    }).join("");
}
...
function renderAdminUser() {
    ...
    container.innerHTML = `
        ...
        <button class="block-big-btn danger" data-act="delete-user" data-username="${escapeHtml(u.username)}">🗑️ DELETE USER</button>
    `;
}
...
function handleShareApp() {
  ...
  if (navigator.share) {
    ...
    navigator.clipboard.writeText(shareText).then(() => showToast("✓ Link copy ho gaya!", "success"));
  }
}
...
/* ============================================================
   MADADGAR APP — JavaScript Core Engine (PART 3 - FINAL)
   ============================================================ */
...
async function reportPost(id) {
  ...
  try {
    ...
    showToast("✓ Post report ho gayi. Shukriya!", "success");
    renderHome();
  } catch (err) {
...
function openEditPostModal(id) {
  ...
  modal.innerHTML = `
    <div class="modal-card" style="background:white; padding:20px; border-radius:8px; width:90%; max-width:400px;">
      <h3 style="margin-top:0;">✏️ Edit Post (Admin)</h3>
